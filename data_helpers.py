...
def logger_fn(name, input_file, level=logging.INFO):
    ...
    Returns:
        The logger.
    """
    logger = logging.getLogger(name)#名称
    logger.setLevel(level)#级别
    log_dir = os.path.dirname(input_file)#路径
    ...
    fh.setFormatter(formatter)#将信息计入文件
...
def get_out_dir(option, logger):
    ...
    if option == 'T':
        ...
        out_dir = os.path.abspath(os.path.join(os.path.curdir, "runs", timestamp))#创建新目录
...
def create_prediction_file(output_file, data_id, true_labels, predict_labels, predict_scores):
    ...
    with open(output_file, 'w') as fout:#写入预测结果
...
def create_metadata_file(word2vec_file, output_file):
    ...
    wv = KeyedVectors.load(word2vec_file, mmap='r')#获得word2vec数据
...
def load_word2vec_matrix(word2vec_file):
    ...
    embedding_matrix = np.zeros([vocab_size, embedding_size])#初始化
    for key, value in word2idx.items():#将词向量放入嵌入矩阵中
        if key == "_UNK":
