...
class TextHARNN(object):
    ...
    def __init__(
        ...
        def _fc_layer(input_x, name=""):
            ...
            return fc_out
        #局部层（局部预测）得到logits（未经过激活），scores（激活后每个类别概率），visual（对下一层类别判断的输入信息，影响）
        ...
        # Embedding Layer 嵌入层将word，层次类别转换为向量
        ...
        # Bi-LSTM Layer 双向长短时，用词向量进行前后文预测得到整个语句的意思，向量表示
        ...
        # Add dropout丢弃正则化，即将那些预测出来概率小的有概率视为0
